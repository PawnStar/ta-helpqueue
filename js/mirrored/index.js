...
{
	$("#getHelpButton").removeClass("btn-danger");
	$("#getHelpButton").removeClass("btn-warning");
	$("#getHelpButton").removeClass("btn-success");
	...
	if(queueActive)
	{
		...
		{
			$("#getHelpButton").attr('disabled', 'disabled');	
			$("#getHelpButton").html('Enter a question or click Pass Off');
		}
		...
		$("#getHelpButton").addClass("btn-success");
	...
	{
		$("#getHelpButton").addClass("btn-warning");
		$("#getHelpButton").html('The Help Queue is not currenlty active');
		...
		$("#getHelpButtonNoQuestion").html('The Help Queue is not currenlty active');
		$("#passOffButtonNoQuestion").addClass("btn-warning");
		$("#passOffButtonNoQuestion").html('The Help Queue is not currenlty active');
...
function createFilteredGraphs(data, startTime, endTime)
{
	...
	$(".graphUnit").empty();
	if(daySplitting)
	{//the period is longer than one day. So make the scale in days
	...
	{//the period is less than a day, scale to day
	...
	*/
	...
	for(var key in sortedData)
	{
		...
		function averageAndMedianValues(index, oneTimeUnitStats, averageWaitTimes,medianGraphValues,numberHelpCountPerHour,numberPassOffCountPerHour, timeWithTAAvg)
		{
			...
			return {'averageWaitTimes':averageWaitTimes, 
	...
	for (index in data)
	{
		if(!isNaN(index))
		{
			...
			if(obj.passOff != 'true')
			{
				...
				returnedData.push({'name':convertNetIdToName(obj.netId), 
...
function showChatNote()
{
	var msg = "New chat session opened!"
	if (typeof Notification !== 'undefined') 
	{
		...
		Notification.requestPermission(function (permission) 
		{
			if (permission === 'granted')
			{
				var notification = new Notification(msg, {
					body: 'Please respond to chat in your browser!',
					tag:"MyNotify"
				});
...
function updateUI(data)
{
	...
	else if(data.hasOwnProperty("spot") && data.hasOwnProperty("enqueueTime"))
	{
		if(pauseUpdate == false)
		{
			...
			else //in line
			{
				...
				$("#getHelpButton").addClass("btn-danger");
	...
	else if(data.hasOwnProperty("list"))
	{
		...
		$.each(data.list, function(index, obj)
		{
			...
			var questionColumn = '<div class="col-xs-5">' + obj.question +
			'&nbsp;<button id="removeButton' + obj.netId +'" onClick=chatWith('+ params +') class="btn btn-default btn-xs fa fa-comment-o"> Chat</button>' +
			'</div>';
...
function chatWith(u,name){
	chat = new Chat(u);
	chat.notify(user);
	chat.enter(user);
	$('#chat-title').html("Chat with: " + name);
	$('#chatModal').modal({backdrop: 'static',keyboard: false});
}

function stuChat(){
	//showChatNote(); the alerts are annoying
	if(($("#chatModal").data('bs.modal') || {}).isShown) return;
	chat.clear();
	chat.enter(user);
	$('#chatModal').modal({backdrop: 'static',keyboard: false});
}
